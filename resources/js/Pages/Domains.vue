...
let paginatedSpoofList = computed(() => {
  ...
  //   const spoof = sort; // Assuming sortList returns an array of spoof objects

  //   if (spoof.current_scan_status === 'not_scanned' && spoof.phashes === 'processing') {
  //     i += 1;
  //   } else {
  //     sorted.push(sort); // Add sort into sorted array
  //     i += 1;
  //   }
  // });
        ...
        <div v-if="showTable && index === indexStore" class="items-center overflow-auto bg-transparent border-collapse lg:ml-4 md:ml-4 md:-mr-3 smooth " :class="{ 'smoothDropDown': showTable }" id="smoothDropDown">
          <div :class="{ 'table-container.open': showTable, 'table-container' : !showTable}">
              ...
              <tbody>
                <!-- <tr class=" tableRow
                      ...
                      v-for="(spoof, index) in paginatedSpoofList" :key="index"
                          ...
                          > -->
                <tr class=" tableRow
                      transition-colors duration-300 hover:!bg-yellow-200 border-solid border-white text-black" v-for="(spoof, index) in paginatedSpoofList" :key="index" :class="{ 'hidden': spoof.spoofed_domain == domain.domain_name }" :style="{'background-color': methods.overallRatingLabel(methods.overallRating(spoof), spoof) === 'High' && spoof.spoofed_domain == domain.domain_name ? '#EBF7E7' : methods.overallRatingLabel(methods.overallRating(spoof), spoof) === 'High' ? '#FDE6E6' : methods.overallRatingLabel(methods.overallRating(spoof), spoof) === 'medium' ? '#FEF1E9' : '#EBF7E7'}">
                  <!--  :style="{ background: index % 2 !== 1 ? '#FDE6E6' : '#F7E6E4' }" -->
                  ...
                  <td class="font-bold text-left py-auto" v-else :style="{'color': methods.overallRatingLabel(methods.overallRating(spoof), spoof) === 'High' ? '#ED0707' : methods.overallRatingLabel(methods.overallRating(spoof), spoof) === 'medium' ? '#F7610D' : '#3AAC11'}">
                    {{ methods.overallRatingLabel(methods.overallRating(spoof), spoof) }}
                    <!-- {{spoof.overallRatingLabel}} -->
                  </td>
                  <td class="py-1">
                    <Link :href="'/spoof/view/' + spoof.id" class="w-16 py-1 mx-auto transition-all duration-150 ease-linear tableButton focus:outline-none" :style="{'background-color': methods.overallRatingLabel(methods.overallRating(spoof), spoof) === 'High' ? '#F89999' : methods.overallRatingLabel(methods.overallRating(spoof), spoof) === 'medium' ? '#FCBE9C' : '#AEDD9D'}">
                    View
                    </Link>
                  </td>
                </tr>
            ...
            <div class="bg-white" style="width: 100%; height: 330px" v-if="paginatedSpoofList.length === 0">
              <div class="mx-auto ">
                <h1 class="pt-6 text-center max-w-60 h6" :class="{
