...
function reported(){
    reporte.value = props.spoofData.spoofed_domain + ' is reported for takedown. Do you fill it should be reported?';
     openModalt();
    //  Toast.fire({
    //         icon: 'error',
  ...
  <AuthenticatedLayout v-if="props.userid === 1" class="overflow-scroll fontFamily" style="height:100vh; background: #FFF;">
    ...
    <div class="flex justify-between mt-6 w-100">
      <div class="relative ml-6">
        <button class="absolute w-40 h-12 px-3 rounded-tr-full bg-dark tabsText" style="">{{props.domain.domain_name}}</button>
          ...
          <div class="w-full overflow-auto" v-if=" heading ==  'Risk Rating'">
            ...
            <div class="flex mt-2 hover:bg-yellow-300 DetailsTableRow" v-else v-for="riskRating in riskRatings" :key="riskRating.value">
              ...
              <div class="mt-3 ml-50 w-100 DetailsTableRowText" v-else>
                ...
                <div v-else>{{ spoofData[riskRating.value]  }} </div>
                <div v-if="riskRating.value === 'High' || riskRating.value === 'low' || riskRating.value === 'medium' || riskRating.key === 'Age'" class="capitalize"> {{riskRating.value}}</div>
                <div v-if="riskRating.value === 'No '" class="capitalize"> {{riskRating.value}}Risk</div>
    ...
    <div class="flex justify-between pt-3 mb-6">
      <div class="flex justify-between w-56 pl-7">
        <button class="my-2 buttons buttonsText hover:bg-yellow-300 sm:hidden lg:flex md:flex hidding" title="Mark As No Risk" @click="openModalr">Mark Safe</button>
        ...
        <button v-if="isValidTakedown" class="my-auto bg-yellow-300 buttons buttonsText hover:bg-yellow-400" as='button' @click="reported">Report</button>
        <!-- || spoofData.spoof_status_new == 'inprogress'|| response  || spoofData.spoof_status_new == 'completed' || spoofData.spoof_status_new == 'inprogress' <i class="text-xl fa-solid fa-ban"></i>-->
              ...
              <div class="">
                <div class="mx-auto bg-white modelStyle" style="">
                  ...
                  <div class="relative flex my-2 modelText">
                    ...
                    <img class="absolute m-2" :src="'/assets/systemImages/bookmark.svg'" />
                    <div class="my-auto">By clicking 'Mark as Safe', you are indicating that <span class="text-yellow-500">{{spoofData.spoofed_domain}} will be ignored</span> . Are you sure you want to proceed?</div>
                  </div>
                  <div class="relative w-100">
              ...
              <div class="">
                <div class="mx-auto bg-white modelStyle" style="">
                  ...
                  <div class="relative flex my-2 modelText">
                    <img class="pr-3 " :src="'/assets/systemImages/Promo.svg'" />
                    <img class="absolute m-2" :src="'/assets/systemImages/bookmark.svg'" />
                    <div class="my-auto"><span v-if="reporte == ''">Hi, <span class="text-yellow-500">{{spoofData.spoofed_domain}}</span>is not reported by user for takedown. Do you want to proceed with reporting?</span><span v-if="reporte != ''">{{reporte}} </span></div>
                  </div>
                  <div class="relative w-100">
                    <!-- <button class="float-right px-4 mr-3 bg-gray-300 buttons buttonsText" @click="closeModal" type="button">cancel</button> -->
                    ...
                    <Link :href="'/spoof/report/' + spoofData.id" v-if="reporte != ''" class="float-right px-4 bg-yellow-300 buttons buttonsText" preserve-scroll>
                    <div class="px-4 my-auto">Report</div>
                    </Link>
                  </div>
                </div>
              </div>
